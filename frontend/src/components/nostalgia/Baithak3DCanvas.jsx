 ...
 */
function createParchiTexture(title, englishTitle, score, accentColor, bgColor = '#fffcf7') {
 ...
 */
function createFoldedParchiMesh(frontTexture, backColorHex, index, width = 2.2, height = 3.0, creaseAngleDeg = 20) {
...
export default function Baithak3DCanvas() {
    ...
    useEffect(() => {
        ...
        const ambientLight = new THREE.AmbientLight(isDark ? 0x2e1d4e : 0xfff6ea, isDark ? 1.6 : 2.4);
        ...
        const sphereGlowLight = new THREE.PointLight(isDark ? 0xf59e0b : 0xe11d48, isDark ? 4.5 : 3.0, 25);
        ...
        const equatorRing = new THREE.Mesh(new THREE.TorusGeometry(3.1, 0.04, 16, 90), armillaryRingMat);
        equatorRing.rotation.x = Math.PI / 2;
        const meridianRing = new THREE.Mesh(new THREE.TorusGeometry(3.1, 0.04, 16, 90), armillaryRingMat);
        meridianRing.rotation.y = Math.PI / 3;
        const latitudeRing1 = new THREE.Mesh(new THREE.TorusGeometry(2.5, 0.03, 16, 80), armillaryRingMat);
        latitudeRing1.position.y = 1.5;
        latitudeRing1.rotation.x = Math.PI / 2;
        const latitudeRing2 = new THREE.Mesh(new THREE.TorusGeometry(2.5, 0.03, 16, 80), armillaryRingMat);
        ...
        chitsData.forEach((item, index) => {
            const texture = createParchiTexture(item.title, item.eng, item.pts, item.color, isDark ? '#faf3e8' : '#ffffff');
            const chit = createFoldedParchiMesh(texture, item.backColor, index, 2.2, 3.0, 20);
