...
export default function MonsoonOverlay({ active }: { active: boolean }) {
  const { easterEggs, cardAnimations } = useNostalgia();
  if (!active || !easterEggs || !cardAnimations) return null;

  return (
    <div className="pointer-events-none fixed inset-0 z-30 bg-gradient-to-b from-indigo/10 to-transparent" aria-hidden="true">
