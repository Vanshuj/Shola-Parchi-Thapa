...
export default function LabelEditor({ labels, onChange }) {
    ...
    return (<div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
      {CARD_TYPES.map((type) => (<div key={type} className="flex items-center gap-4 rounded-lg border border-outline-variant p-4">
          <LabelPreviewCard type={type} label={labels[type]}/>
          <div className="flex-1">
            <div className="flex items-center gap-2">
              <Input aria-label={`Label for ${type}`} value={labels[type]} maxLength={MAX_LABEL_LENGTH} onChange={(e) => handleChange(type, e.target.value)} error={errors[type]}/>
