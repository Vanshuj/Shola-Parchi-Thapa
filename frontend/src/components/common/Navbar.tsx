...
const NAV_LINKS: { path: string; label: string; to: string }[] = [
...
export default function Navbar() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 w-full bg-surface-container-low/95 dark:bg-surface-container-low/90 dark:border-b dark:border-outline-variant/30 shadow-[0_4px_16px_rgba(46,21,0,0.08)] dark:shadow-[0_4px_24px_rgba(0,0,0,0.5)] backdrop-blur-md transition-colors duration-300">
      <div className="flex h-20 w-full items-center justify-between gap-space-md px-space-md lg:px-margin">
        ...
        <div className="flex items-center gap-space-sm lg:gap-space-md">
          {room && (
            <Link
              to={`/lobby/${room.roomCode}`}
              className="hidden items-center gap-space-xs rounded-lg bg-surface-container-highest px-space-sm py-space-xs sm:flex hover:bg-surface-container-high transition-colors"
            >
          ...
          {isAuthenticated ? (
            <>
              <Link
                to="/settings"
                title="Settings &amp; Baithak Preferences"
            ...
            <>
              <Link
                to="/login"
                className="rounded-lg px-space-md py-space-sm font-label-lg text-label-lg text-on-surface-variant hover:text-on-surface"
              >
                Log in
              </Link>
              <Link
                to="/signup"
                className="rounded-lg bg-primary px-space-md py-space-sm font-label-lg text-label-lg font-bold text-on-primary shadow-sm hover:bg-primary-container"
              >
                Sign up
              </Link>
            </>
