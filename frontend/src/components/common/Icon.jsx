...
export default function Icon({ name, className = '', size = 24, filled = false }) {
    return (<span className={`material-symbols-outlined select-none ${className}`} style={{ fontSize: size, fontVariationSettings: filled ? '"FILL" 1' : undefined }} aria-hidden="true">
      {name}
    </span>);
}
