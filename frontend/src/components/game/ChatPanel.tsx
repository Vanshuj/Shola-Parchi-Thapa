  ...
  return (
    <div className="flex h-[32rem] w-full lg:w-80 flex-col rounded-xl bg-surface-container-low shadow-sm shrink-0">
        ...
        <div className="flex flex-wrap gap-space-xs">
          {QUICK_TAUNTS.map((t) => (
            <button
              ...
              className="flex items-center gap-1 rounded-full bg-surface-container-highest px-space-sm py-1 font-label-sm text-label-sm text-on-surface hover:bg-secondary-container/40 transition-colors"
            >
            ...
            <div key={i} className={isYou ? 'text-right' : 'text-left'}>
              ...
              <p
                className={`inline-block rounded-xl px-space-sm py-space-xs font-body-md text-body-md max-w-[85%] ${
                  isYou ? 'bg-primary text-on-primary' : 'bg-surface-container-lowest text-on-surface'
                }`}
              >
                {m.text}
              </p>
      ...
      <form onSubmit={handleSubmit} className="flex gap-space-xs border-t border-outline-variant p-space-sm">
        ...
        <button
          type="submit"
          className="w-10 h-10 rounded-lg bg-primary text-on-primary flex items-center justify-center shrink-0 hover:bg-primary-container transition-colors"
        >
