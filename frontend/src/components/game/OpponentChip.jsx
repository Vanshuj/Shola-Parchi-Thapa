...
export default function OpponentChip({ opponent, isCurrentTurn, align = 'center' }) {
    const alignClass = align === 'start' ? 'items-start' : align === 'end' ? 'items-end' : 'items-center';
    return (<div className={`flex flex-col ${alignClass}`}>
      <div className={`relative bg-surface-container-lowest/95 dark:bg-surface-container-high/90 border border-outline-variant/50 p-space-sm rounded-2xl shadow-xl flex flex-col items-center gap-space-xs w-32 transition-all duration-300 ${isCurrentTurn
            ? 'ring-4 ring-secondary-container/90 glow-gold-pulse shadow-[0_0_30px_rgba(254,166,25,0.45)] scale-105'
            : 'hover:shadow-2xl'}`}>
        {isCurrentTurn && (<div className="absolute -top-3 bg-secondary-container text-on-secondary-container px-space-xs py-0.5 rounded-full font-label-sm text-label-sm font-bold shadow-md flex items-center gap-1 glow-gold animate-bounce">
