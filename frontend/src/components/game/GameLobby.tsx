...
export default function GameLobby({ room, isHost, onStart, onLeave }: GameLobbyProps) {
    ...
    <div className="max-w-5xl mx-auto">
        ...
        <div className="flex flex-wrap items-center gap-space-sm sm:gap-space-md">
          ...
          <div className="flex items-center gap-1 bg-surface-container-lowest px-space-sm py-1 rounded-lg border border-outline-variant">
            ...
            <button
              ...
              className="p-1 rounded hover:bg-surface-container text-secondary transition-colors"
            >
          ...
          <button
            ...
            className="px-space-sm py-1 rounded-full bg-surface-container-highest hover:bg-surface-container-high text-on-surface-variant font-label-sm text-label-sm font-semibold flex items-center gap-1 transition-colors"
          >
      ...
      <div className="bg-surface-container-lowest px-space-lg py-space-xl rounded-b-xl shadow-sm">
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-space-md mb-space-xl">
          {room.seatedUsernames.map((name, i) => (
            <div key={name} className="rounded-xl bg-surface-container p-space-md flex flex-col items-center gap-space-xs text-center">
          ...
          {Array.from({ length: emptySeats }).map((_, i) => (
            <div
              key={`empty-${i}`}
              className="rounded-xl border-2 border-dashed border-outline-variant p-space-md flex flex-col items-center justify-center gap-space-xs text-center text-on-surface-variant"
            >
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-space-md pt-space-lg border-t border-outline-variant">
          <button
            type="button"
            onClick={onLeave}
            className="px-space-lg py-space-sm rounded-lg bg-surface-container text-on-surface-variant font-label-lg text-label-lg font-bold hover:bg-surface-container-high transition-colors"
          >
            Leave Baithak
          </button>
          {isHost ? (
            <button
              ...
              onClick={onStart}
              disabled={!canStart}
              className="px-space-xl py-space-md rounded-lg bg-primary text-on-primary font-label-lg text-label-lg font-bold shadow-md hover:shadow-xl disabled:opacity-40 disabled:cursor-not-allowed flex items-center gap-space-xs"
            >
