...
export default function GameTable({ state, selectedCardId, onSelectCard, onConfirmPass }) {
    ...
    const currentTurnName = realOpponents.find((o) => Number(o.id) === Number(state.currentTurnPlayerId))?.username ?? (isYourTurn ? 'You' : 'Table');
    ...
    return (<div className="flex flex-col items-center gap-space-lg w-full">
      {top.length > 0 && (<div className="flex justify-center gap-space-md">
          {top.map((opp) => (<OpponentChip key={opp.id} opponent={opp} isCurrentTurn={state.currentTurnPlayerId === opp.id}/>))}
        </div>)}

      <div className="flex items-center justify-between w-full max-w-3xl gap-space-md">
        <div className="flex flex-col gap-space-md">
          {left.map((opp) => (<OpponentChip key={opp.id} opponent={opp} isCurrentTurn={state.currentTurnPlayerId === opp.id} align="start"/>))}
        </div>

        {/* Center circle with radiant glow aura */}
        <div className="relative flex flex-col items-center justify-center shrink-0">
          <div className={`w-72 h-72 sm:w-80 sm:h-80 rounded-full bg-gradient-to-tr from-surface-container to-surface-container-high dark:from-surface-container-low dark:to-surface-container shadow-2xl flex items-center justify-center relative p-3 transition-all duration-500 ${isYourTurn
            ...
            {/* Turn status banner */}
            <div className={`absolute -top-6 px-space-md py-space-xs rounded-full shadow-lg flex items-center gap-space-xs z-30 whitespace-nowrap transition-all duration-300 ${isYourTurn
        ...
        <div className="flex flex-col gap-space-md">
          {right.map((opp) => (<OpponentChip key={opp.id} opponent={opp} isCurrentTurn={state.currentTurnPlayerId === opp.id} align="end"/>))}
        </div>
      </div>

      {/* Selection action bar */}
      <div className={`flex items-center gap-space-md bg-surface-container-lowest dark:bg-surface-container-high border border-outline-variant/60 px-space-lg py-space-xs rounded-full shadow-2xl transition-all duration-300 ${selectedCard ? 'opacity-100 scale-100 glow-gold' : 'opacity-0 scale-95 pointer-events-none'}`}>
        ...
        <button type="button" onClick={onConfirmPass} disabled={!isYourTurn} className="px-space-md py-space-xs rounded-full bg-primary text-on-primary hover:bg-primary-container font-label-lg text-label-lg shadow-md flex items-center gap-space-xs transition-all active:scale-95 disabled:opacity-40 hover:glow-kumkum">
      ...
      <div className="w-full max-w-4xl flex flex-col items-center">
        <div className="flex items-center justify-between w-full px-space-md mb-space-xs">
          <div className={`flex items-center gap-space-xs font-label-sm text-label-sm font-bold transition-colors ${isYourTurn ? 'text-primary dark:text-secondary-container' : 'text-secondary'}`}>
        ...
        <div className={`w-full rounded-2xl border-2 border-dashed transition-all duration-500 bg-surface-container/60 dark:bg-surface-container-low/80 py-space-md backdrop-blur-sm ${isYourTurn
