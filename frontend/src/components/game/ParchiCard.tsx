...
const TYPE_COLORS: Record<CardDTO['type'], string> = {
...
export default function ParchiCard({ card, selected, disabled, onClick, rotationSeed = 0 }: ParchiCardProps) {
  ...
  return (
    <motion.button
      type="button"
      layoutId={cardAnimations ? `parchi-${card.id}` : undefined}
      onClick={onClick}
      disabled={disabled}
      aria-pressed={selected}
      aria-label={`Parchi labeled ${card.label}`}
      whileHover={!disabled && cardAnimations ? { y: -8, scale: 1.05, rotate: 0 } : undefined}
      whileTap={!disabled ? { scale: 0.96 } : undefined}
      style={{ rotate: `${rotation}deg` }}
      className={`relative w-20 h-28 md:w-24 md:h-32 shrink-0 rounded-lg shadow-md transition-all duration-300 focus-visible:outline focus-visible:outline-2 focus-visible:outline-primary disabled:opacity-40 disabled:cursor-not-allowed ${
        selected
