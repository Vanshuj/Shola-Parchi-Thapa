    ...
    <div className="max-w-6xl mx-auto px-space-md lg:px-margin py-space-xl">
      ...
      {podium.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-space-md mb-space-xl">
          {podium.map((entry, i) => (
            <div key={entry.rank} className={`rounded-xl border p-space-lg ${PODIUM_STYLE[i].bg} ${i === 0 ? 'sm:order-2 sm:-translate-y-3' : i === 1 ? 'sm:order-1' : 'sm:order-3'}`}>
          ...
          <table className="w-full text-left">
            ...
            <tbody>
              {rest.map((entry) => {
                ...
                return (
                  <tr key={entry.rank} className={`border-b border-outline-variant/50 ${isYou ? 'bg-secondary-container/10' : ''}`}>
