...
export default function GameTablePage() {
    ...
    if (!state)
        return <Loader label="Dealing the parchis…"/>;
    ...
    return (<div className="w-full">
      ...
      <div className="px-space-md lg:px-margin py-space-lg flex flex-col lg:flex-row items-start justify-center gap-space-lg">
        <div className="flex-1 flex justify-center w-full">
          <GameTable state={state} selectedCardId={selectedCardId} onSelectCard={(id) => setSelectedCardId((prev) => (prev === id ? null : id))} onConfirmPass={handleConfirmPass}/>
        </div>
      ...
      {isFinished && (<WinnerBanner winnerUsername={winnerUsername} isYou={state.winnerId === userId} onGoToResults={() => navigate(`/results/${gameId}`)}/>)}
    </div>);
}
