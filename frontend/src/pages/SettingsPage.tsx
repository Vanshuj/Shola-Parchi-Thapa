...
export default function SettingsPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-space-md lg:px-margin py-space-xl">
        ...
        {TIERS.map((tier) => (
          <section key={tier.title}>
            <div className="flex items-center justify-between mb-space-sm">
              ...
              {tier.subtitle && (
                <span className="font-body-sm text-body-sm text-on-surface-variant hidden sm:block">{tier.subtitle}</span>
              )}
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-space-md">
              {tier.toggles.map((toggle) => (
                <label
                  key={toggle.key}
                  className="rounded-xl bg-surface-container-low p-space-md flex items-start gap-space-sm cursor-pointer"
                >
