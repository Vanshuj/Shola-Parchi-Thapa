...
const SUIT_META: Record<CardType, { label: string; stamp: string; accent: string }> = {
...
export default function CustomizeParchisPage() {
  ...
  const [draft, setDraft] = useState<LabelMap | null>(null);
  ...
  useEffect(() => {
    if (labels) setDraft(labels);
  }, [labels]);

  if (loading || !draft) return <Loader label="Loading your parchis…" />;
  ...
  function handleLabelChange(type: CardType, value: string) {
    setDraft((prev) => (prev ? { ...prev, [type]: value.slice(0, MAX_LABEL_LENGTH) } : prev));
  }

  return (
    <div className="max-w-7xl mx-auto px-space-md lg:px-margin py-space-xl">
      {/* Header */}
      <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-space-md mb-space-lg">
        ...
        <div className="flex gap-space-sm shrink-0">
          <button
            type="button"
            onClick={handleReset}
            className="px-space-md py-space-sm rounded-lg bg-surface-container text-on-surface-variant font-label-lg text-label-lg font-bold hover:bg-surface-container-high transition-colors flex items-center gap-space-xs"
          >
          ...
          <button
            type="button"
            onClick={handleSave}
            className="px-space-md py-space-sm rounded-lg bg-primary text-on-primary font-label-lg text-label-lg font-bold shadow-sm hover:shadow-md transition-all flex items-center gap-space-xs"
          >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-space-md">
          {BUILTIN_PRESETS.map((preset) => (
            <button
              key={preset.id}
              type="button"
              onClick={() => setDraft(preset.labels)}
              className="text-left rounded-xl bg-surface-container-low hover:bg-surface-container-high transition-colors p-space-md shadow-sm"
            >
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-space-md mb-space-xl">
          {CARD_TYPES.map((type, i) => (
            <div key={type} className="rounded-xl bg-surface-container-low shadow-sm overflow-hidden">
              <div className="p-space-md">
                ...
                <div className="flex items-center gap-space-xs">
                  <input
                    ...
                    className="flex-1 rounded-lg border border-outline-variant bg-surface-container-lowest px-space-sm py-2 font-body-md text-body-md text-on-surface outline-none focus:border-primary"
                  />
      ...
      <div className="rounded-xl bg-surface-container-low p-space-lg">
        ...
        <div className="flex items-end justify-center -space-x-6 hover:space-x-1 transition-all duration-300">
          {CARD_TYPES.map((type, i) => (
            <ParchiCard key={type} card={{ id: `preview-${type}`, type, label: draft[type] || 'Untitled' }} rotationSeed={i} />
          ))}
        </div>
      </div>
